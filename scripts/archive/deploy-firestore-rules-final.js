    ...
    const req = https.request(url, options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200 || res.statusCode === 201) {
          ...
          releaseRuleset(projectId, rulesetName, accessToken);
        } else {
...
function releaseRuleset(projectId, rulesetName, accessToken) {
  const releaseName = `projects/${projectId}/releases/cloud.firestore`;
  const url = `https://firebaserules.googleapis.com/v1/${releaseName}?release_name=${releaseName}`;
  ...
  const req = https.request(url, options, (res) => {
    ...
    res.on('end', () => {
      if (res.statusCode === 200) {
        ...
        console.log('   Try adding a country again in the UI');
        process.exit(0);
      } else {
